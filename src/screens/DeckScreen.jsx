// ═══════════════════════════════════════════════════════════
// OPEN MAT — DECK SCREEN (THE NOTEBOOK)
// Position-first grouping. Dead ends stare back at you in red.
// No animations. No gamification. Honest feedback.
// ═══════════════════════════════════════════════════════════
...
export default function DeckScreen({ profile, onProfileUpdate }) {
  ...
  const allTechs = useMemo(() => {
    const beltOrd = { white: 1, blue: 2, purple: 3, brown: 4, black: 5 };
    const lvl = beltOrd[profile?.belt] || 1;
    return Object.values(G.techniques).map(t => ({
      ...t,
      locked: (beltOrd[t.belt_unlock] || 1) > lvl,
    }));
  }, [profile?.belt]);
  ...
  const deadEndFamilies = useMemo(() => {
    const map = {};
    for (const de of deadEnds) {
      if (!map[de.family]) map[de.family] = [];
      map[de.family].push(de);
    }
    return map;
  }, [deadEnds]);
  ...
  return (
    <div style={{ padding: '20px 16px' }}>
      ...
      <div style={{
        ...
        <div style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, background: `${currentArch.color}10`, border: `1.5px solid ${currentArch.color}25`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <ArchIcon id={currentArch.id} s={20} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: F.display, fontSize: 16, color: T.text }}>{currentArch.name}</div>
          <div style={{ fontFamily: F.mono, fontSize: 8, color: T.dim }}>{currentArch.animal} — {currentArch.strength}</div>
        </div>
      ...
      {tab === 'deck' && (
        <div style={{
          padding: '10px 14px', background: '#FFFFFF', border: `1px solid ${T.border}`,
          borderRadius: 8, marginBottom: 12, fontFamily: F.mono, fontSize: 10,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            <span style={{ fontFamily: F.display, fontSize: 20, color: T.text }}>
            ...
            <div style={{ flex: 1, display: 'flex', gap: 4, alignItems: 'center' }}>
              {Object.entries(typeCounts).map(([type, count]) => {
                ...
                return <div key={type} style={{ flex: count, height: 6, background: tc?.color || T.dim, borderRadius: 3, minWidth: 4 }} title={`${tc?.label}: ${count}`} />;
              })}
            </div>
          </div>
          {/* Dead ends — quiet red. Like a coach circling something in red pen. */}
          <div style={{ color: deadEnds.length > 0 ? T.sub : T.td, fontSize: 9 }}>
            {deadEnds.length > 0
              ? `Dead ends: ${deadEnds.length} — ${deadEnds.map(d => d.name).join(', ')}`
              : 'No dead ends'
            }
          </div>
        </div>
      ...
      {tab === 'library' && (
        <div style={{ marginBottom: 12 }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search moves..."
            style={{ width: '100%', padding: '10px 14px', background: '#FFFFFF', border: `1.5px solid ${T.border}`, borderRadius: 8, fontFamily: F.body, fontSize: 14, color: T.text, outline: 'none' }} />
        </div>
      )}
        ...
        {TYPE_FILTERS.map(f => {
          ...
          return (
            <button key={f.key} onClick={() => setFilter(f.key)} style={{
              ...
              {f.label}{count != null && tab === 'deck' ? ` ${count}` : ''}
            </button>
          );
        })}
      </div>
      ...
      {FAMILY_ORDER.map(family => {
        ...
        const familyDeadEnds = tab === 'deck' ? (deadEndFamilies[family] || []) : [];
        const hasFg = !!fg;
        const techCount = hasFg ? Object.values(fg.positions).reduce((s, p) => s + p.techs.length, 0) : 0;

        // In deck tab: show family even if empty, IF it has dead ends
        if (!hasFg && familyDeadEnds.length === 0) return null;

        const unlockedCount = tab === 'library' && hasFg
          ...
          <div key={family} style={{ marginBottom: 6 }}>
            ...
            <button onClick={() => toggle(familyKey)} style={{
              ...
              <span style={{
                ...
                color: familyDeadEnds.length > 0 ? T.sub : T.textSecondary,
                flex: 1,
              }}>
                {FAMILY_LABELS[family] || family}
                {familyDeadEnds.length > 0 && (
                  <span style={{ fontFamily: F.mono, fontSize: 9, color: T.sub, marginLeft: 8 }}>
                    {familyDeadEnds.length} dead end{familyDeadEnds.length > 1 ? 's' : ''}
                  </span>
                )}
              </span>
              {tab === 'library' && unlockedCount != null && (
                <span style={{ fontFamily: F.mono, fontSize: 9, color: T.dim }}>{unlockedCount}/{techCount}</span>
              )}
              {tab === 'deck' && techCount > 0 && (
            ...
            {!isCollapsed && (
              <div style={{
                ...
                {/* Dead end positions — empty rows that stare back */}
                {familyDeadEnds.map(de => (
                  <div key={de.posId} style={{ padding: '12px 16px', borderBottom: `1px solid #F3F4F6` }}>
                    <div style={{ fontFamily: F.mono, fontSize: 10, color: T.sub }}>
                      {de.name} — NO MOVES
                    </div>
                    <div style={{ fontFamily: F.body, fontSize: 12, color: T.muted, marginTop: 4 }}>
                      You can reach this position but have no way out. Add an escape.
                    </div>
                    <div onClick={() => { setTab('library'); setFilter('escape'); }} style={{
                      fontFamily: F.mono, fontSize: 10, color: T.muted, marginTop: 4, cursor: 'pointer',
                    }}>
                      → Browse escapes
                    </div>
                  </div>
                ...
                {/* Technique rows */}
                {hasFg && Object.entries(fg.positions).map(([posId, posGroup]) => (
                  <div key={posId}>
                    <div style={{
                    ...
                    {posGroup.techs.map((t, i) => {
                      ...
                      return (
                        <RowCard key={techId || i} move={t} type={t.type} tier={t.tier || 'known'}
                          gp={GP_COSTS[t.type] || 1} isInDeck={isIn} locked={t.locked}
                          onClick={() => setExpandedMove({ ...t, techId })}
                          belt={profile?.belt} playerArchetype={profile?.archetype} timesUsed={t.times_used} />
                      );
                    })}
                  </div>
                ))}
      ...
      {grouped['other'] && (
        <div style={{ marginBottom: 6 }}>
          <div style={{ padding: '10px 14px', fontFamily: F.body, fontSize: 14, fontWeight: 500, color: T.textSecondary, background: '#FFFFFF', border: `1px solid ${T.border}`, borderRadius: 8 }}>Other</div>
          <div style={{ border: `1px solid ${T.border}`, borderRadius: 10, overflow: 'hidden', marginTop: 4, background: '#FFFFFF' }}>
            {Object.entries(grouped['other'].positions).map(([posId, posGroup]) => (
              <div key={posId}>
                ...
                {posGroup.techs.map((t, i) => (
                  <RowCard key={t.technique_id || t.id || i} move={t} type={t.type} tier={t.tier || 'known'}
                    gp={GP_COSTS[t.type] || 1} isInDeck={tab === 'deck' || deckTechIds.has(t.id || t.technique_id)}
                    locked={t.locked} onClick={() => setExpandedMove(t)} belt={profile?.belt}
                    playerArchetype={profile?.archetype} timesUsed={t.times_used} />
                ))}
              </div>
            ))}
          </div>
      ...
      {Object.keys(grouped).length === 0 && deadEnds.length === 0 && (
      ...
      {expandedMove && (
        <div style={{
          ...
          <div onClick={e => e.stopPropagation()}>
            <FullCard move={expandedMove} type={expandedMove.type} tier={expandedMove.tier || 'known'}
              gp={GP_COSTS[expandedMove.type] || 1} locked={expandedMove.locked} requiredBelt={expandedMove.belt_unlock}
              ...
              belt={profile?.belt} playerArchetype={profile?.archetype} timesUsed={expandedMove.times_used} />
          </div>
        </div>
      )}
      ...
      {showArchPicker && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(248,248,251,0.97)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '18px 20px 10px', borderBottom: `1px solid ${T.border}`, flexShrink: 0 }}>
            ...
            <div style={{ fontFamily: F.mono, fontSize: 9, color: T.dim, marginTop: 4 }}>Your deck stays the same. Mastery bonuses change.</div>
          </div>
            ...
            {ARCHETYPES.map(a => {
              ...
              return (
                <div key={a.id} onClick={() => !isCurrent && setPendingArch(a.id)} style={{
                  ...
                  <div style={{ width: 40, height: 40, borderRadius: '50%', flexShrink: 0, background: `${a.color}10`, border: `1.5px solid ${a.color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <ArchIcon id={a.id} s={22} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontFamily: F.display, fontSize: 17, color: T.text }}>{a.name}</span>
                      <span style={{ fontFamily: F.mono, fontSize: 9, color: T.dim }}>{a.animal}</span>
                      {isCurrent && <span style={{ fontFamily: F.mono, fontSize: 7, color: a.color, background: `${a.color}12`, padding: '1px 6px', borderRadius: 3 }}>CURRENT</span>}
                    </div>
                    <div style={{ fontFamily: F.mono, fontSize: 9, color: T.dim, marginTop: 2 }}>{a.strength} · {a.weakness}</div>
