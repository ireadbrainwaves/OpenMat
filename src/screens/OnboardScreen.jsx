...
const STARTER_DECKS = {
  wrestler: [
    { id: "takedown_artist", name: "Takedown Artist", desc: "Double legs, singles, top control finishes", moves: 24, subs: 4, sweeps: 0, transitions: 12, escapes: 4, takedowns: 4 },
    { id: "grind_pin", name: "Grind & Pin", desc: "Body lock, pressure passing, mount submissions", moves: 25, subs: 5, sweeps: 1, transitions: 10, escapes: 5, takedowns: 4 },
  ],
  guard_puller: [
    { id: "sweep_machine", name: "Sweep Machine", desc: "Scissor sweeps, hip bumps, constant reversals", moves: 24, subs: 4, sweeps: 8, transitions: 6, escapes: 4, takedowns: 2 },
    { id: "guard_assassin", name: "Guard Assassin", desc: "Triangles, armbars, attack from your back", moves: 25, subs: 7, sweeps: 4, transitions: 6, escapes: 5, takedowns: 3 },
  ],
  leg_locker: [
    { id: "heel_hook_hunter", name: "Heel Hook Hunter", desc: "Inside sankaku, ashi entries, heel hooks", moves: 24, subs: 6, sweeps: 2, transitions: 8, escapes: 5, takedowns: 3 },
    { id: "ankle_specialist", name: "Ankle Lock Specialist", desc: "Straight ankle locks, toe holds, 50/50", moves: 24, subs: 5, sweeps: 3, transitions: 8, escapes: 5, takedowns: 3 },
  ],
  pressure_passer: [
    { id: "smash_pass", name: "Smash Pass", desc: "Knee slice, smash pass, heavy top", moves: 25, subs: 4, sweeps: 1, transitions: 12, escapes: 4, takedowns: 4 },
    { id: "knee_cutter", name: "Knee Cutter", desc: "Knee slice, leg drag, speed passing", moves: 24, subs: 3, sweeps: 1, transitions: 13, escapes: 4, takedowns: 3 },
  ],
  submission_hunter: [
    { id: "sub_sniper", name: "Submission Sniper", desc: "Armbars, triangles, chain attacks", moves: 25, subs: 8, sweeps: 3, transitions: 6, escapes: 5, takedowns: 3 },
    { id: "choke_artist", name: "Choke Artist", desc: "RNC, guillotine, darce, collar chokes", moves: 24, subs: 7, sweeps: 2, transitions: 7, escapes: 5, takedowns: 3 },
  ],
  scrambler: [
    { id: "chaos_agent", name: "Chaos Agent", desc: "Arm drags, back takes, constant movement", moves: 25, subs: 3, sweeps: 4, transitions: 10, escapes: 5, takedowns: 3 },
    { id: "berimbolo_bandit", name: "Berimbolo Bandit", desc: "Inversions, berimbolo, tricky entries", moves: 24, subs: 4, sweeps: 5, transitions: 8, escapes: 4, takedowns: 3 },
  ],
};
...
export default function OnboardScreen({ user, onComplete }) {
  ...
  const NameStep = () => (
    <div style={{ animation: "fadeUp 0.3s ease-out both" }}>
      ...
      <Btn full onClick={() => setStep(1)} disabled={!name.trim()}>Continue</Btn>
    </div>
  );
  ...
  const ArchetypeStep = () => (
    <div style={{ animation: "fadeUp 0.3s ease-out both" }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <div style={{ flex: 1 }}><Btn full onClick={() => setStep(2)} disabled={!archetype}>Continue</Btn></div>
      </div>
    </div>
  );
  ...
  const BeltStep = () => (
    <div style={{ animation: "fadeUp 0.3s ease-out both", textAlign: "center", paddingTop: 20 }}>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
        ...
        <Btn onClick={() => setStep(3)}>Got It</Btn>
      </div>
    </div>
  );
    ...
    return (
      <div style={{ animation: "fadeUp 0.3s ease-out both" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <div style={{ flex: 1 }}><Btn full onClick={handleComplete} disabled={!deck || saving}>{saving ? "Setting up..." : "Start Rolling"}</Btn></div>
        </div>
      </div>
    );
  };

  const steps = [NameStep, ArchetypeStep, BeltStep, DeckStep];
