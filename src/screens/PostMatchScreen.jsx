// ═══════════════════════════════════════════════════════════
// OPEN MAT — POST MATCH SCREEN (LIGHT MODE)
// Results, Elo change, technique progress, stripe/belt progress
// ═══════════════════════════════════════════════════════════
...
export default function PostMatchScreen({ profile, match, onRematch, onHome }) {
  ...
  return (
    <div style={{ padding: "20px", animation: "fadeUp 0.3s ease-out", background: T.bg, minHeight: "100vh" }}>
      {/* Result headline */}
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <div style={{ fontFamily: F.display, fontSize: 48, color: resultColor, lineHeight: 1 }}>
          {won ? (winMethod === "submission" ? "Submission" : "Victory") : "Defeat"}
        </div>
        {winMethod === "submission" && subTechName && (
          <div style={{ fontFamily: F.display, fontSize: 22, color: subColor, marginTop: 4 }}>{subTechName}</div>
        )}
        <div style={{ fontFamily: F.mono, fontSize: 10, color: T.dim, marginTop: 6 }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 16, marginBottom: 20, fontFamily: F.mono }}>
        ...
        <div style={{ fontSize: 24, color: resultColor, alignSelf: "center", fontFamily: F.display }}>
          {won ? "+" : ""}{eloChange}
        </div>
        <div style={{ textAlign: "center" }}>
